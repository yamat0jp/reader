...
function createWindow () {
  ...
  if (os.type() === 'Linux') {
    ...
    mainWindow = new BrowserWindow({ width: 0, height: 0, show: false, webPreferences: { nodeIntegration: true, contextIsolation: false }, useContentSize: true, icon: icoPath})
  } else {
    mainWindow = new BrowserWindow({ width: 0, height: 0, show: false, webPreferences: { nodeIntegration: true, contextIsolation: false }, useContentSize: true})
  }
  ...
  let prjPath = path.resolve(__dirname, './epub.html')
...
ipcMain.handle("ipc-browserwindow-create", (event, args) => {
  ...
  let bw;
  if (!obj.noparent) {
    bw = new BrowserWindow({parent: parentWindow, width: 0, height: 0, show: false, useContentSize: true, webPreferences: { nodeIntegration: true, contextIsolation: false },
	...
	bw = new BrowserWindow({width: 0, height: 0, show: false, useContentSize: true, webPreferences: { nodeIntegration: true, contextIsolation: false },
                              fullscreenable: obj.fullscreenable, fullscreen: obj.fullscreen, kiosk: obj.kiosk, resizable: obj.resizable,
                              minWidth: obj.minWidth, minHeight: obj.minHeight, modal: obj.modal, icon: img});   
  }
  ...
  if (!obj.noparent) {
	...
	browserRelations[bw.id]	= parentWindow.id;
	 
    bw.on('show', (event) => {
	  let sibling = BrowserWindow.fromId(browserRelations[event.sender.id]);
	  if (sibling) {
        sibling.webContents.send('ipc-browserwindow-show-event' + event.sender.id);
	  }
    });
    ...
    bw.on('focus', (event) => {
      let sibling = BrowserWindow.fromId(browserRelations[event.sender.id]);
	  if (sibling) {
        sibling.webContents.send('ipc-browserwindow-activate-event' + event.sender.id);
	  }
    });
    ...
    bw.on('hide', (event) => {
      let sibling = BrowserWindow.fromId(browserRelations[event.sender.id]);
	  if (sibling) {
        sibling.webContents.send('ipc-browserwindow-hide-event' + event.sender.id);
	  }
    });
    ...
    bw.on('resize', (event) => {
      let sibling = BrowserWindow.fromId(browserRelations[event.sender.id]);
	  if (sibling) {
        sibling.webContents.send('ipc-browserwindow-resize-event' + event.sender.id, [bw.getSize()[0], bw.getSize()[1]]);
	  }
    });
    ...
    bw.on('minimize', (event) => {
      let sibling = BrowserWindow.fromId(browserRelations[event.sender.id]);
	  if (sibling) {
        sibling.webContents.send('ipc-browserwindow-minimize-event' + event.sender.id);
	  }
    });
    ...
    bw.on('maximize', (event) => {
      let sibling = BrowserWindow.fromId(browserRelations[event.sender.id]);
	  if (sibling) {
        sibling.webContents.send('ipc-browserwindow-maximize-event' + event.sender.id);
	  }
    });
    ...
    bw.on('blur', (event) => {
      let sibling = BrowserWindow.fromId(browserRelations[event.sender.id]);
	  if (sibling) {
        sibling.webContents.send('ipc-browserwindow-deactivate-event' + event.sender.id);
	  }
    });
    ...
    bw.on('enter-full-screen', (event) => {
      let sibling = BrowserWindow.fromId(browserRelations[event.sender.id]);
	  if (sibling) {
        sibling.webContents.send('ipc-browserwindow-fullscreen-event' + event.sender.id);
	  }
    });
    ...
    bw.on('leave-full-screen', (event) => {
      let sibling = BrowserWindow.fromId(browserRelations[event.sender.id]);
	  if (sibling) {
        sibling.webContents.send('ipc-browserwindow-exit-fullscreen-event' + event.sender.id);
	  }
    });
    ...
    bw.on('close', (event) => {
      let sibling = BrowserWindow.fromId(browserRelations[event.sender.id]);
	  if (sibling) {
        sibling.webContents.send('ipc-browserwindow-close-event' + event.sender.id);
	  }
      allWindows.forEach(function(item, index, object) {
...
ipcMain.handle("ipc-border-window", (event, args) => {
  if (args == "sizable") {
    BrowserWindow.fromWebContents(event.sender).setResizable(true);
  } else {
    BrowserWindow.fromWebContents(event.sender).setResizable(false);
  }
})
